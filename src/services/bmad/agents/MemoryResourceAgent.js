...
export default class MemoryResourceAgent {
  ...
  async optimizeBeforeProcessing() {
    console.log('[ORDX:MemoryResourceAgent] 🧹 ========== OTIMIZAÇÃO DE MEMÓRIA (ANTES) ==========')
    const beforeLength = this.conversationHistory.length
    const beforeMemory = this.estimateMemoryUsage()
    
    console.log('[ORDX:MemoryResourceAgent] 📊 Estado atual:', {
    ...
    if (this.conversationHistory.length > this.maxHistoryMessages) {
      ...
      console.log('[ORDX:MemoryResourceAgent] 🗑️ Histórico limpo:', {
        before: beforeLength,
      ...
      console.log('[ORDX:MemoryResourceAgent] ✅ Tamanho do histórico OK:', this.conversationHistory.length, 'mensagens')
    }
    
    console.log('[ORDX:MemoryResourceAgent] ✅ Otimização concluída')
  }

  async optimizeAfterProcessing(feedback) {
    console.log('[ORDX:MemoryResourceAgent] 📊 ========== OTIMIZAÇÃO DE MEMÓRIA (DEPOIS) ==========')
    console.log('[ORDX:MemoryResourceAgent] 📝 Input:', {
    ...
    console.log('[ORDX:MemoryResourceAgent] 💾 Uso de memória:', {
    ...
    if (this.memoryUsage > 80) {
      ...
      this.cleanupCache()
      const afterMemory = this.estimateMemoryUsage()
      console.log('[ORDX:MemoryResourceAgent] 🧹 Limpeza de cache acionada:', {
        before: beforeLength,
        after: this.conversationHistory.length,
        removed: beforeLength - this.conversationHistory.length,
      ...
      console.log('[ORDX:MemoryResourceAgent] ✅ Uso de memória dentro do limite')
    }
  ...
  async updateHistory(entry) {
    console.log('[ORDX:MemoryResourceAgent] 📝 ========== ATUALIZANDO HISTÓRICO ==========')
    console.log('[ORDX:MemoryResourceAgent] 📝 Entrada:', {
    ...
    if (this.conversationHistory.length > 100) {
      ...
      console.log('[ORDX:MemoryResourceAgent] 🗑️ Histórico reduzido:', {
        before: beforeLength,
        after: this.conversationHistory.length,
        removed: beforeLength - this.conversationHistory.length
      })
    } else {
      console.log('[ORDX:MemoryResourceAgent] ✅ Histórico atualizado:', {
        before: beforeLength,
        after: this.conversationHistory.length,
    ...
    console.log('[ORDX:MemoryResourceAgent] 📚 Histórico completo (últimas 3):', 
  ...
  async getConversationHistory() {
    console.log('[ORDX:MemoryResourceAgent] 📖 ========== OBTENDO HISTÓRICO ==========')
    console.log('[ORDX:MemoryResourceAgent] 📊 Estado:', {
    ...
    console.log('[ORDX:MemoryResourceAgent] 📤 Retornando', history.length, 'entradas recentes do histórico')
    console.log('[ORDX:MemoryResourceAgent] 📋 Resumo do histórico:', history.map((h, i) => ({
      index: i,
