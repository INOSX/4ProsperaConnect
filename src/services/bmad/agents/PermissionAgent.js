...
export default class PermissionAgent {
  async checkPermission(intent, user, params) {
    console.log('[BMAD:PermissionAgent] 🔐 ========== VERIFICANDO PERMISSÕES ==========')
    console.log('[BMAD:PermissionAgent] 📝 Input:', {
      intent: intent,
      userId: user?.id,
    ...
    if (!user) {
      console.log('[BMAD:PermissionAgent] ❌ Permissão negada: Usuário não autenticado')
      const result = {
        allowed: false,
        reason: 'Usuário não autenticado'
      }
      console.log('[BMAD:PermissionAgent] 📤 Resultado:', JSON.stringify(result, null, 2))
      return result
    }

    try {
      // Buscar role do usuário
      console.log('[BMAD:PermissionAgent] 🔍 Buscando role do usuário...')
      const clientResult = await ClientService.getClientByUserId(user.id)
      console.log('[BMAD:PermissionAgent] 📥 Resultado do ClientService:', {
      ...
      if (!clientResult.success || !clientResult.client) {
        console.log('[BMAD:PermissionAgent] ❌ Permissão negada: Cliente não encontrado')
        const result = {
          allowed: false,
      ...
      console.log('[BMAD:PermissionAgent] 👤 Role do usuário:', {
        role: userRole,
      ...
      console.log('[BMAD:PermissionAgent] 🔍 Verificando permissões para intent:', intent)
      ...
      console.log('[BMAD:PermissionAgent] 📊 Verificação de permissão:', {
        intent: intent,
        allowed: allowed,
      ...
      console.log('[BMAD:PermissionAgent]', allowed ? '✅ Permissão concedida' : '❌ Permissão negada', 'para intent:', intent)
      ...
      console.log('[BMAD:PermissionAgent] ✅ ========== VERIFICAÇÃO CONCLUÍDA ==========')
      console.log('[BMAD:PermissionAgent] 📤 Resultado:', JSON.stringify(result, null, 2))
      
      return result
    } catch (error) {
      console.error('[BMAD:PermissionAgent] ❌ ========== ERRO NA VERIFICAÇÃO ==========')
      console.error('[BMAD:PermissionAgent] ❌ Erro:', error)
      console.error('[BMAD:PermissionAgent] ❌ Stack:', error.stack)
      const result = {
        allowed: false,
        reason: 'Erro ao verificar permissões'
      }
      console.log('[BMAD:PermissionAgent] 📤 Resultado (erro):', JSON.stringify(result, null, 2))
      return result
    }
  }
}
