...
export default class SuggestionAgent {
  ...
  async generateSuggestions(text, intentResult, actionResult, history) {
    console.log('[FLX:SuggestionAgent] 💡 ========== GERANDO SUGESTÕES ==========')
    console.log('[FLX:SuggestionAgent] 📝 Input:', {
    ...
    console.log('[FLX:SuggestionAgent] 🔍 Buscando sugestões baseadas em padrões para intent:', intent)
    if (this.suggestionPatterns[intent]) {
      suggestions.push(...this.suggestionPatterns[intent])
      console.log('[FLX:SuggestionAgent] ✅ Encontradas', this.suggestionPatterns[intent].length, 'sugestões baseadas em padrões:', 
        this.suggestionPatterns[intent].map(s => s.text))
    } else {
      console.log('[FLX:SuggestionAgent] ⚠️ Nenhuma sugestão de padrão encontrada para intent:', intent)
      console.log('[FLX:SuggestionAgent] 📋 Intents disponíveis:', Object.keys(this.suggestionPatterns))
    }

    // Sugestões genéricas baseadas no histórico
    if (history && history.length > 0) {
      console.log('[FLX:SuggestionAgent] 🔍 Analisando histórico para sugestões...')
      const lastAction = history[history.length - 1]
      console.log('[FLX:SuggestionAgent] 📚 Última ação no histórico:', {
      ...
      if (lastAction.intent === 'create_company') {
        ...
        console.log('[FLX:SuggestionAgent] ✅ Adicionada sugestão baseada em histórico:', historySuggestion.text)
      }
    } else {
      console.log('[FLX:SuggestionAgent] ℹ️ Nenhum histórico disponível para análise')
    }

    console.log('[FLX:SuggestionAgent] 📊 Total de sugestões coletadas:', suggestions.length)
    console.log('[FLX:SuggestionAgent] 🔄 Ordenando por relevância...')
    ...
    console.log('[FLX:SuggestionAgent] ✅ ========== SUGESTÕES GERADAS ==========')
    console.log('[FLX:SuggestionAgent] 📤 Resultado:', {
    ...
    console.log('[FLX:SuggestionAgent] 📋 Sugestões completas:', JSON.stringify(finalSuggestions, null, 2))
